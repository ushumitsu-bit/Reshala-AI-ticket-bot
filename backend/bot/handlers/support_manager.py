...
async def close_ticket_by_client_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Менеджер закрывает тикет по client_id (close_ticket_by_client:client_id)"""
    query = update.callback_query
    data = query.data or ""
    ...
    try:
        ...
        # Ищем тикет по client_id
        ticket = db.tickets.find_one({"client_id": client_id, "is_removed": {"$ne": True}, "status": {"$ne": "closed"}})
        if ticket:
            ticket_id_str = str(ticket["_id"])
            result = await ticket_service.close_ticket(ticket_id_str, user_id=None, is_manager=True)
        else:
            result = {"ok": False, "error": "Тикет не найден"}
    except Exception as e:
    ...
    if result.get("ok"):
        ...
        if client_id_val:
            topic_by_client = context.application.bot_data.get("support_topic_by_client", {})
            topic_by_client.pop(client_id_val, None)
        if thread_id and support_group_id:
            thread_to_client = context.application.bot_data.get("support_thread_to_client", {})
            thread_to_client.pop((support_group_id, thread_id), None)
        ...
        await query.message.reply_text(f"❌ Ошибка: {result.get('error')}")
